...
- **多语言本地化（稳妥常态）**：先填 `work-tasks/{slug}/03-locale-briefs.md` → 母版一语 → **逐语或每批 ≤3 语**按 brief 独立重写（禁止 `Translate to {lang}`；禁止 en/zh 后脚本灌其余语的同构交付）→ 禁词表 + ≥3 轮抽查；「待母语抽查」须重写/确认后清掉才可 `i18n-done`。`lint:seo` ≠ 本地化完成。细则：`tool-i18n-localization.mdc`。
