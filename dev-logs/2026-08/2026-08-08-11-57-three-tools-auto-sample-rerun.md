...
Summary: Re-ran color-from-image, image-merge, and image-overlay to auto-execute sample pipelines on page load per the new rule.

[question]
将最近生成的3个工具按照新规则重新跑一次

[try to solve]
Aligned the three newest image tools with `tool-creation.mdc`「进页样例须实际跑出结果」:
...
Updated each `work-tasks/*/02-tool-info.md` with the 进页样例 spec. `lint:seo` OK.
