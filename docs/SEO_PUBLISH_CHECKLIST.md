...
1. `npm run build:site` — 全量生成各语言首页 / About、完整 `public/sitemap.xml`，并预渲染所有工具 HTML + gzip。`npm run build:site:full` 保留为显式全量别名。（sitemap **默认不含**关于/隐私/条款/联系；筛选见 [`ops/README.md`](../ops/README.md) §4.0，`npm run sitemap` / `sitemap:ui`）
...
9. **R2 / Worker 版本**：本机已配置 `.env` S3 凭据（`cp .env.example .env`，见 [`ops/worker-r2-ops.md`](../ops/worker-r2-ops.md) §3.1）→ `npm run deploy`（全量 build + hash 增量 `upload:r2` + `verify:r2`）→ **git push**（CF 拉 GitHub）→ `npm run verify:r2:live`；确认 `GET /api/ops/pages-build` 的 `aligned: true`。排障、重建基线或怀疑远端缺对象时用 `npm run deploy:full`。

`npm run deploy` 会通过 `predeploy` 自动执行 1–2，并继续 hash 增量 upload / verify；**不**本机 `wrangler deploy`。R2 上传由本地 `.html.gz` sha256 与上次成功上传 manifest 的 `fileHashes` 对比决定，不看最新 commit、最新 push 或未提交 git diff。勿只 push 不 upload（工具页 HTML 不在 Git）。无 S3 `.env` 时 upload 会回退 wrangler 逐文件 put（很慢）。
