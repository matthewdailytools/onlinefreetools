...
**选题战略（2026-08-20）**：[2026-08-20-long-tail-gap-strategy.md](./2026-08-20-long-tail-gap-strategy.md) — 不与已有流量站抢已占位大词；主攻其未覆盖/极薄的长尾与语言缺口。已有 GSC 展示的大词仅做 CTR 收割，不作进攻立项。头词工具占位时可用 **§3.3 G `title_gap_fallback`**：同簇长尾在 SERP title 中未出现 → 作 slug/H1（一簇一 URL）。**§3.3 H（全站）**：slug/H1 须为用户**实际使用场景**任务句（情境+动作+结果），进页默认即该场景；平台/渠道只是情境的一种拆法。禁止品类头词与 `*-pack` 工程名当主 slug；按作业类型聚类，禁止近义标签拆 URL。
...
对齐 `tool-i18n-seo.mdc` 与长尾策略 **§3.3 H**：

- **默认**：同一工具页用 Use cases + FAQ + **一键场景芯片**覆盖同作业类型的多个真实使用场景（一带多场景）
- **slug/H1**：用户实际会搜的**使用场景任务句**（情境+动作+结果）；进页默认预设 = H1 场景（一步到位）。平台/渠道是情境的一种，不是唯一拆法。禁止 `social-share-image-pack`、`cidr-calculator` 这类品类/工程名
- **可拆独立 URL**：仅当**主控件/约束**不同（如 cidrsubnet 求值 vs 单块展开；OG 链接卡 vs 封面安全区），且独立满足 IG 配额
- **不可拆**：仅换同义词、品牌名或平台名、同一套公式/裁切
- **自检**：去掉 title 后正文是否仍明显不同？无「使用场景」表不得标 `build`
