...
## 2026-09-21 补充快照：待建队列复核

按当前代码复核：274 个 catalog，sound-editor 主题 60 项。旧状态表仅为历史，不再作为未实现数量。声音 F1–F13 的 99 个明确 slug 中 39 个尚未注册；页面存在不等于能力验收。优先 P0 核对文件转写/字幕与 Hub 合同，再评估 BPM、节拍器、噪声发生器；跨主题优先扫描 PDF→TXT、批量 EXIF 清除、批量 PDF 水印、SVG 优化。完整清单、条件和依据见 [待建工具与优先级复核](./reviews/2026-09-21/tool-backlog-priority.md)。本轮没有新增 SERP 候选、不改变词池 verdict、不新建立项目录；推荐不等于 long_gap 认定。
