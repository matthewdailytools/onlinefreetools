...
const P = 'tool_add_an_audio_track_to_a_video';
...
/**
 * 给本地视频添加一条音轨并下载成片（F7 V2）。
 * 管线：读视频+音频 → 解码音 → capture 画面（丢原音）+ MediaStream 音轨 → MediaRecorder。
 * ≠ 去音轨（V1）；≠ 抽音（A1）；≠ YouTube；诚实重编码限制。
 * @param opts.lang 当前 UI 语言
 * @param opts.defaultLang 默认（无前缀）语言
 * @param opts.enabledLangs 启用语言列表
 */
export const renderAddAnAudioTrackToAVideoPage = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/add-an-audio-track-to-a-video';
	/** 当前语言的规范 URL 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'add-an-audio-track-to-a-video',
			currentAnchor: '#add-audio',
		}),
		id: 'toolNav',
	});
	...
	/** 工具主面板与结果区 markup。 */
	const contentHtml = `
    <div id="add-audio" class="tool-page-heading mb-3"><h1 class="h4">${tr('title')}</h1><p>${tr('desc')}</p></div>
    <section class="tool-panel" id="aaPanel">
      <label class="tool-dropzone mb-3" id="aaVideoDrop" for="aaVideo"><input id="aaVideo" type="file" accept="video/*,.mp4,.webm,.mov,.m4v,video/mp4,video/webm,video/quicktime"><span class="tool-dropzone-title">${tr('choose_video')}</span><span class="tool-dropzone-hint">${tr('hint_video')}</span><span id="aaVideoName" class="tool-dropzone-file"></span></label>
      <label class="tool-dropzone mb-3" id="aaAudioDrop" for="aaAudio"><input id="aaAudio" type="file" accept="audio/*,.wav,.mp3,.m4a,.aac,.ogg,.flac,audio/wav,audio/mpeg,audio/mp4"><span class="tool-dropzone-title">${tr('choose_audio')}</span><span class="tool-dropzone-hint">${tr('hint_audio')}</span><span id="aaAudioName" class="tool-dropzone-file"></span></label>
      <div class="tools-bar d-flex flex-wrap mb-3" style="gap:.5rem">
        <button id="aaConvert" class="btn btn-primary" type="button">${tr('convert')}</button>
        <button id="aaDownload" class="btn btn-outline-primary" type="button" disabled>${tr('download')}</button>
        <button id="aaSample" class="btn btn-outline-secondary" type="button">${tr('sample')}</button>
        <button id="aaClear" class="btn btn-outline-secondary" type="button">${tr('clear')}</button>
      </div>
      <details class="mb-3" open><summary>${tr('advanced')}</summary>
        <p class="form-text mt-2">${tr('settings_hint')}</p>
      </details>
      <div id="aaHud" class="bcw-hud mb-3" role="status" aria-live="polite" hidden>
        <div class="bcw-hud-top"><div class="bcw-hud-spin" aria-hidden="true"></div><div class="bcw-hud-pct" id="aaPct">0%</div><div class="bcw-hud-copy"><div class="bcw-hud-title">${tr('progress')}</div><div class="bcw-hud-step" id="aaStep"></div><div class="bcw-hud-time" id="aaTime"></div></div></div>
        <div class="progress" style="height:1.35rem"><div id="aaBar" class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div><span class="bcw-hud-sheen" aria-hidden="true"></span></div>
        <ol class="bcw-hud-steps"><li data-step="read">${tr('read')}</li><li data-step="decode">${tr('decode')}</li><li data-step="merge">${tr('merge')}</li><li data-step="write">${tr('write')}</li></ol><div class="bcw-hud-url" id="aaCurrent"></div>
      </div>
      <div id="aaEmpty" class="aa-empty mb-3" role="status">${tr('empty_state')}</div>
      <div id="aaOutput" hidden><h2 class="h5">${tr('preview')}</h2><p id="aaResult"></p><video id="aaPreview" controls preload="metadata" playsinline style="max-width:100%" aria-label="${tr('preview')}"></video></div>
    </section>`;
  ...
  (function(){
    ...
    /** 视频文件输入。 */
    const videoInput = $('aaVideo');
    /** 音频文件输入。 */
    const audioInput = $('aaAudio');
    /** 进度 HUD。 */
    const hud = $('aaHud');
    /** 预览 video。 */
    const preview = $('aaPreview');
    /** 空状态提示。 */
    const emptyState = $('aaEmpty');
    /** HUD 步骤顺序。 */
    ...
    function discardOut(){
      ...
      preview.load();
      $('aaOutput').hidden = true;
      $('aaResult').textContent = '';
      $('aaDownload').disabled = true;
    }
    /** 丢弃源视频 URL。 */
    ...
    function lock(on){
      busy = on;
      $('aaConvert').disabled = on;
      $('aaSample').disabled = on;
      $('aaClear').disabled = on;
      videoInput.disabled = on;
      audioInput.disabled = on;
      if (!on) $('aaDownload').disabled = !outputUrl;
    }
    /**
     * 更新 HUD。
    ...
    function progress(pct, stepKey){
      ...
      $('aaPct').textContent = value + '%';
      $('aaBar').style.width = value + '%';
      $('aaBar').setAttribute('aria-valuenow', String(value));
      const label = stepKey === 'done' ? M.done : (M[stepKey] || stepKey);
      $('aaStep').textContent = label;
    ...
    function fail(key){
      ...
      $('aaPct').textContent = '—';
      $('aaBar').style.width = '0%';
      $('aaBar').setAttribute('aria-valuenow', '0');
      hud.setAttribute('role', 'alert');
      $('aaStep').textContent = M[key] || M.failed;
    }
    /**
     * 是否为可尝试的本地视频。
     * @param {File} file 候选
    ...
    function chooseVideo(file){
      ...
      videoFile = file;
      $('aaVideoName').textContent = fill(M.video_label, { name: file.name });
      refreshEmpty();
    }
    /**
     * 选择音频。
    ...
    function chooseAudio(file){
      ...
      audioFile = file;
      $('aaAudioName').textContent = fill(M.audio_label, { name: file.name });
      refreshEmpty();
    }
    /**
     * 等待 video 元数据就绪。
    ...
    async function addAudio(){
      ...
      $('aaCurrent').textContent = (videoFile.name || '') + ' + ' + (audioFile.name || '');
      started = performance.now();
      const clock = () => {
        $('aaTime').textContent = fill(M.elapsed, { s: ((performance.now() - started) / 1000).toFixed(1) });
      };
      clock();
      ...
      try {
        ...
        $('aaOutput').hidden = false;
        $('aaResult').textContent = fill(M.result, {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        const videoDone = new Promise((res, rej) => {
          ...
          vRec.onstop = () => {
            ...
            res(new File([blob], (M.sample_name || 'add-audio-sample') + '.webm', { type: 'video/webm' }));
          };
        });
        vRec.start(100);
        ...
        for (let i = 0; i < frames; i++){
          data[i] = Math.sin(2 * Math.PI * 440 * (i / RATE)) * 0.2;
        }
        ...
        const audioFileOut = new File([bytes], (M.sample_name || 'add-audio-sample') + '.wav', { type: 'audio/wav' });
        try {
    ...
    $('aaVideoDrop').addEventListener('dragover', event => event.preventDefault());
    $('aaVideoDrop').addEventListener('drop', event => {
    ...
    $('aaAudioDrop').addEventListener('dragover', event => event.preventDefault());
    $('aaAudioDrop').addEventListener('drop', event => {
      event.preventDefault();
      if (busy) return;
      const files = event.dataTransfer && event.dataTransfer.files;
    ...
    $('aaConvert').addEventListener('click', addAudio);
    $('aaSample').addEventListener('click', loadSample);
    $('aaClear').addEventListener('click', clearAll);
    $('aaDownload').addEventListener('click', () => {
      if (!outputUrl || busy) return;
      const ext = (outMime.indexOf('mp4') >= 0) ? 'mp4' : 'webm';
      const link = document.createElement('a');
      link.href = outputUrl;
      link.download = (videoFile && videoFile.name ? videoFile.name.replace(/\\.[^.]+$/, '') : 'merged') + '-with-audio.' + ext;
	...
	/** catalog 元数据。 */
	const toolMeta = getToolBySlug('add-an-audio-track-to-a-video');
	/** related 等扩展区块。 */
