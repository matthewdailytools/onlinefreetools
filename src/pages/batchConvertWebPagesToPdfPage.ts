/**
 * 批量把公开网页转成 PDF：URL 列表串行代抓 + 本标签页 html2pdf，成功项打 ZIP。
...
const PREFIX = 'tool_batch_convert_web_pages_to_pdf';

/**
 * 网页→PDF 截图客户端：把消毒后的 HTML 写入隐藏 iframe，处理图片/溢出后交给 html2pdf.js。
 * 与 convert-html-to-pdf 使用同一套 /asset 代理与截图规则；内联在本页以免隔离门禁拦共享文件。
 * 返回注入到 extraBodyHtml 的 IIFE 源码，挂到 window.OftHtmlToPdfCapture。
...
const htmlToPdfCaptureClientScript = (): string => `
    (function () {
      ...
      /**
       * 把 HTML 转成 PDF Blob（A4 纵向）。
       * @param {{html:string,iframe:HTMLIFrameElement,waitMs?:number,onWaiting?:Function,onBeforeCapture?:Function}} opts 输入
       * @returns {Promise<Blob>}
       */
      function convertHtmlToPdfBlob(opts) {
        ...
        if (typeof html2pdf === 'undefined') return Promise.reject(new Error('html2pdf'));
        return writePreviewDocument(iframe, html).then(function (doc) {
          ...
          var liveW = (doc.defaultView && doc.defaultView.innerWidth)
            || ((iframe && iframe.clientWidth) ? iframe.clientWidth : 800);
      ...
      window.OftHtmlToPdfCapture = {
        convertHtmlToPdfBlob: convertHtmlToPdfBlob,
      };
    })();
`;
 ...
 */
export const renderBatchConvertWebPagesToPdfPage = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/batch-convert-web-pages-to-pdf';
	/** 当前语言下的 canonical 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'batch-convert-web-pages-to-pdf',
  ...
  <style>
    ...
    <div class="tool-panel">
      ...
      <div class="d-flex align-items-center tools-bar mb-2 flex-wrap gap-2">
        ...
        <label class="bcw-wait-field d-flex align-items-center gap-1 mb-0 small text-nowrap" for="bcwLoadWait">
  ...
  <script>
    ...
    (function () {
       ...
       */
      function htmlToPdfBytes(html, onPhase) {
        var cap = window.OftHtmlToPdfCapture;
        if (!cap || typeof cap.convertHtmlToPdfBlob !== 'function') {
          return Promise.reject(new Error('html2pdf'));
        }
        return cap.convertHtmlToPdfBlob({
          html: html,
          iframe: captureFrame,
      ...
      function convertOne(item, usedNames, onPhase) {
        ...
        return job.then(function (html) {
          ...
          return yieldUi().then(function () { return htmlToPdfBytes(html, onPhase); });
        }).then(function (bytes) {
          ...
          var label = /empty|Missing url|Invalid/i.test(eStr) ? msg.urlEmpty
            ...
            : /html2pdf|dompurify/i.test(eStr) ? msg.loadFail
            : msg.convertFail;
	...
	const toolMeta = getToolBySlug('batch-convert-web-pages-to-pdf');
