/**
 * 批量把公开网页转成无损 PNG：URL 列表串行代抓 + 本标签页 html2canvas（视口/透明底/倍率/整页或首屏），成功项打 ZIP。
 * 首屏只放 URL 列表与动作行；视口/背景/倍率/整页首屏/等待收进「高级设置（可选）」，默认不打开也能 Convert。
...
const PREFIX = 'tool_batch_convert_web_pages_to_png';

/**
 * 网页→PNG 截图客户端：把消毒后的 HTML 写入隐藏 iframe，处理图片/溢出后交给 html2canvas → PNG。
 * 与 convert-html-to-pdf 使用同一套 /asset 代理与截图规则；内联在本页以免隔离门禁拦共享文件。
 * 返回注入到 extraBodyHtml 的 IIFE 源码，挂到 window.OftHtmlToPngCapture。
...
const htmlToPdfCaptureClientScript = (): string => `
    (function () {
      ...
      /**
       * 把 HTML 转成 PNG Blob（选定视口宽度的整页或首屏栅格；无损、可选透明底，不是 JPEG 也不是 A4 PDF）。
       * @param {{html:string,iframe:HTMLIFrameElement,waitMs?:number,viewportW?:number,bgMode?:string,pixelScale?:number,captureMode?:string,onWaiting?:Function,onBeforeCapture?:Function}} opts 输入
       * @returns {Promise<Blob>}
       */
      function convertHtmlToPngBlob(opts) {
        ...
        var bgMode = (opts && opts.bgMode) === 'transparent' ? 'transparent' : 'white';
        var pixelScale = (opts && Number(opts.pixelScale) === 2) ? 2 : 1;
        ...
        return writePreviewDocument(iframe, html).then(function (doc) {
          ...
          var h2cOpts = {
            scale: scale,
            backgroundColor: bgMode === 'transparent' ? null : '#ffffff',
            useCORS: true,
            allowTaint: true,
            logging: false,
            timeout: 30000,
      ...
      window.OftHtmlToPngCapture = {
        convertHtmlToPngBlob: convertHtmlToPngBlob,
      };
    })();
`;
 ...
 */
export const renderBatchConvertWebPagesToPngPage = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/batch-convert-web-pages-to-png';
	/** 当前语言下的 canonical 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'batch-convert-web-pages-to-png',
  ...
  <style>
    ...
    #bcwViewport, #bcwBg, #bcwScale, #bcwCapture { width: auto; min-width: 8rem; }
    ...
    <div class="tool-panel">
      ...
      <details class="bcw-advanced mb-2">
        ...
        <div class="d-flex align-items-center flex-wrap gap-2 mb-2">
          ...
          <label class="bcw-wait-field d-flex align-items-center gap-1 mb-0 small text-nowrap" for="bcwBg">
            <span>${escapeHtml(tx(opts.lang, 'bg_label'))}</span>
            <select id="bcwBg" class="form-select form-select-sm">
              <option value="white" selected>${escapeHtml(tx(opts.lang, 'bg_white'))}</option>
              <option value="transparent">${escapeHtml(tx(opts.lang, 'bg_transparent'))}</option>
            </select>
          </label>
          <label class="bcw-wait-field d-flex align-items-center gap-1 mb-0 small text-nowrap" for="bcwScale">
            <span>${escapeHtml(tx(opts.lang, 'scale_label'))}</span>
            <select id="bcwScale" class="form-select form-select-sm">
              <option value="1" selected>${escapeHtml(tx(opts.lang, 'scale_1x'))}</option>
              <option value="2">${escapeHtml(tx(opts.lang, 'scale_2x'))}</option>
            </select>
          </label>
	 ...
	 * 「载入样例」用本地 HTML 占位跑出两行真实 PNG，不自动请求外网；进页不自动转换。
	 */
	const extraBodyHtml = `
  ${pdfWorkUiClientScript()}
  ...
  <script>
    ...
    (function () {
      ...
      var SAMPLE_URL_1 = 'https://example.com/ui/component-kit';
      /** 样例第二条 URL。 */
      var SAMPLE_URL_2 = 'https://example.com/ui/translucent-overlay';
      ...
      /** PNG 背景：白底或透明。 */
      var bgEl = document.getElementById('bcwBg');
      /** PNG 像素倍率：1 或 2。 */
      var scaleEl = document.getElementById('bcwScale');
      /** 整页或首屏。 */
      ...
      function setBusy(busy) {
        ...
        if (bgEl) bgEl.disabled = !!busy;
        if (scaleEl) scaleEl.disabled = !!busy;
      ...
      /**
       * 读取背景模式（white / transparent）。
       * @returns {string}
       */
      function readBgMode() {
        return bgEl && bgEl.value === 'transparent' ? 'transparent' : 'white';
      }
      ...
      function readPixelScale() {
        return scaleEl && scaleEl.value === '2' ? 2 : 1;
      }
      ...
      function samplePageHtml(heading, body) {
        return '<!doctype html><html><head><meta charset="utf-8"><title>' + heading +
          '</title><style>html,body{margin:0;min-height:100%}body{font-family:system-ui,sans-serif;background-image:linear-gradient(45deg,#d1d5db 25%,transparent 25%),linear-gradient(-45deg,#d1d5db 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#d1d5db 75%),linear-gradient(-45deg,transparent 75%,#d1d5db 75%);background-size:16px 16px;background-position:0 0,0 8px,8px -8px,-8px 0}.bar{height:48px;background:rgba(17,24,39,.92);color:#fff;display:flex;align-items:center;padding:0 20px;font-weight:600;border-bottom:1px solid #111}.card{margin:24px;padding:16px;background:rgba(255,255,255,.72);border:1px solid #111827;border-radius:4px}button{border:1px solid #111827;background:#fff;padding:6px 12px;font:inherit}.hint{font-size:13px;color:#111827;line-height:1.45}</style></head><body><div class="bar">' +
          heading + '</div><div class="card"><p class="hint">' + body + '</p><p><button type="button">Primary</button> <button type="button">Ghost</button></p></div></body></html>';
      }
       ...
       */
      function htmlToPngBytes(html, onPhase) {
        var cap = window.OftHtmlToPngCapture;
        if (!cap || typeof cap.convertHtmlToPngBlob !== 'function') {
          return Promise.reject(new Error('html2canvas'));
        }
        return cap.convertHtmlToPngBlob({
          html: html,
          iframe: captureFrame,
          waitMs: readLoadWaitMs(),
          viewportW: readViewportW(),
          bgMode: readBgMode(),
          pixelScale: readPixelScale(),
      ...
      function convertOne(item, usedNames, onPhase) {
        ...
        return job.then(function (html) {
          ...
          return yieldUi().then(function () { return htmlToPngBytes(html, onPhase); });
        }).then(function (bytes) {
      ...
      /**
       * 加载样例：填入两条 URL，用本地 HTML 转出两份 PNG（不打外网）。
       * 只在用户点击「载入样例」时运行，进页不自动转换。
       */
      function loadSample() {
        urlsEl.value = SAMPLE_URL_1 + '\\n' + SAMPLE_URL_2;
        var html1 = samplePageHtml('Component kit', '1px borders stay sharp on PNG. JPEG would smear the button edges.');
        var html2 = samplePageHtml('Translucent overlay', 'Checkerboard shows through the card when Background is Transparent. This is a lossless PNG, not a JPEG and not an A4 PDF.');
        return runBatch([
	...
	const toolMeta = getToolBySlug('batch-convert-web-pages-to-png');
