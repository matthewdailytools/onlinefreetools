...
const P = 'tool_boost_bass_on_an_mp3';
...
/**
 * 用单节 lowshelf 低音增强，导出 16-bit WAV（S21）。
 * @param opts.lang 当前 UI 语言
 * @param opts.defaultLang 默认（无前缀）语言
 * @param opts.enabledLangs 启用语言列表
 */
export const renderBoostBassOnAnMp3Page = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/boost-bass-on-an-mp3';
	/** 当前语言的规范 URL 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'boost-bass-on-an-mp3',
			currentAnchor: '#bass-boost',
		}),
		id: 'toolNav',
	});
	...
	/** 工具主面板与结果区 markup。 */
	const contentHtml = `
    <div id="bass-boost" class="tool-page-heading mb-3"><h1 class="h4">${tr('title')}</h1><p>${tr('desc')}</p></div>
    <section class="tool-panel" id="bassPanel">
      <label class="tool-dropzone mb-3" id="bassDrop" for="bassFile"><input id="bassFile" type="file" accept=".wav,.mp3,.m4a,.aac,.ogg,audio/*"><span class="tool-dropzone-title">${tr('choose')}</span><span class="tool-dropzone-hint">${tr('hint')}</span><span id="bassName" class="tool-dropzone-file"></span></label>
      <div class="tools-bar d-flex flex-wrap mb-3" style="gap:.5rem">
        <button id="bassConvert" class="btn btn-primary" type="button">${tr('convert')}</button>
        <button id="bassDownload" class="btn btn-outline-primary" type="button" disabled>${tr('download')}</button>
        <button id="bassSample" class="btn btn-outline-secondary" type="button">${tr('sample')}</button>
        <button id="bassClear" class="btn btn-outline-secondary" type="button">${tr('clear')}</button>
      </div>
      <details class="mb-3" open><summary>${tr('advanced')}</summary>
        <p class="form-label mt-2 mb-1">${tr('preset_label')}</p>
        <div class="bass-preset-chips mb-2" id="bassPresetChips" role="group" aria-label="${tr('preset_label')}">
          <button type="button" class="btn btn-sm btn-outline-secondary" data-preset="mild" aria-pressed="false">${tr('preset_mild')}</button>
      ...
      <div id="bassHud" class="bcw-hud mb-3" role="status" aria-live="polite" hidden>
        <div class="bcw-hud-top"><div class="bcw-hud-spin" aria-hidden="true"></div><div class="bcw-hud-pct" id="bassPct">0%</div><div class="bcw-hud-copy"><div class="bcw-hud-title">${tr('progress')}</div><div class="bcw-hud-step" id="bassStep"></div><div class="bcw-hud-time" id="bassTime"></div></div></div>
        <div class="progress" style="height:1.35rem"><div id="bassBar" class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div><span class="bcw-hud-sheen" aria-hidden="true"></span></div>
        <ol class="bcw-hud-steps"><li data-step="read">${tr('read')}</li><li data-step="decode">${tr('decode')}</li><li data-step="boost">${tr('boost')}</li><li data-step="write">${tr('write')}</li></ol><div class="bcw-hud-url" id="bassCurrent"></div>
      </div>
      <div id="bassOutput" hidden><h2 class="h5">${tr('preview')}</h2><p id="bassResult"></p><audio id="bassAudio" controls preload="metadata" style="max-width:100%" aria-label="${tr('preview')}"></audio></div>
    </section>`;
	...
	/** 金标 HUD、预设芯片与布局补充样式。 */
	const extraHeadHtml = `<style>${bcwHudCss({ hudId: 'bassHud', convertBtnId: 'bassConvert' })}#bassHud.is-error{border-color:#b91c1c;background:#fff1f2}#bassHud:not(.is-on) .bcw-hud-spin{animation:none}.bass-preset-chips{display:flex;flex-wrap:wrap;gap:.4rem}.site-footer nav.d-inline{display:inline-flex!important;flex-wrap:wrap;justify-content:center;max-width:100%}</style>`;

	/** 客户端转换管线（读 → 解码 → OfflineAudioContext 单节 lowshelf → 写 16-bit WAV）。 */
  ...
  (function(){
    ...
    /** 工具面板根节点。 */
    const panel = $('bassPanel');
    /** 文件输入。 */
    const fileInput = $('bassFile');
    /** 进度 HUD。 */
    const hud = $('bassHud');
    /** 预览 audio。 */
    const audio = $('bassAudio');
    /** 预设芯片组。 */
    const presetChips = $('bassPresetChips');
    /** 当前选中的输入文件。 */
    let selected = null;
    /** 是否正在转换。 */
    let busy = false;
    ...
    /** 转换开始时间戳（ms）。 */
    let started = 0;
    /** 当前预设键：mild | medium | strong。 */
    let presetKey = 'medium';
    /** 允许的预设键。 */
    const PRESET_KEYS = ['mild', 'medium', 'strong'];
    /**
     * 单节 lowshelf 低音增强预设（≠ 三频段闷声 EQ、≠ 整段增益）。
     * Mild +4 dB / Medium +8 dB / Strong +12 dB，架点约 100 Hz。
     */
    const PRESETS = {
      mild: { type: 'lowshelf', frequency: 100, Q: 0.7, gain: 4 },
      medium: { type: 'lowshelf', frequency: 100, Q: 0.7, gain: 8 },
      strong: { type: 'lowshelf', frequency: 100, Q: 0.7, gain: 12 }
    };
    /** Boost 后软峰值保护上限（线性）。 */
    ...
    function lock(on){
      ...
      $('bassDownload').disabled = on || !outputUrl;
      $('bassConvert').setAttribute('aria-busy', String(on));
    }
    /**
     * 更新 HUD 百分比与步骤高亮。
    ...
    function progress(pct, step){
      $('bassPct').textContent = Math.round(pct) + '%';
      $('bassBar').style.width = pct + '%';
      $('bassBar').setAttribute('aria-valuenow', String(Math.round(pct)));
      $('bassStep').textContent = M[step];
    ...
    function fail(key){
      ...
      $('bassBar').setAttribute('aria-valuenow', '0');
      hud.setAttribute('role', 'alert');
      $('bassStep').textContent = M[key] || M.failed;
    }
    /**
     * 选择或清空输入文件（会丢弃旧输出）。
    ...
    function choose(file){
      ...
      discard();
      selected = file || null;
      $('bassName').textContent = file ? file.name : '';
      hud.hidden = true;
      fileInput.value = '';
    }
    /**
    ...
    /**
     * OfflineAudioContext + 单节 lowshelf，再软峰值保护。
     * @param {AudioBuffer} buffer 解码缓冲
     * @param {string} key 预设键
     */
    async function boostBuffer(buffer, key){
    ...
    async function convert(){
      ...
      $('bassCurrent').textContent = selected.name;
      started = performance.now();
      const clock = () => { $('bassTime').textContent = fill(M.elapsed, { s: ((performance.now() - started) / 1000).toFixed(1) }); };
      clock();
      ...
      try {
        ...
        progress(42, 'boost');
        await yieldUi();
        const boosted = await boostBuffer(decoded, presetKey);
        progress(78, 'write');
        await yieldUi();
        const blob = writeWav(boosted.channels, boosted.rate);
        if (!blob.size) throw Error('err_encoder');
        outputUrl = URL.createObjectURL(blob);
        audio.src = outputUrl;
        $('bassOutput').hidden = false;
        const presetLabel = M['preset_' + presetKey] || presetKey;
        $('bassResult').textContent = fill(M.result, {
          seconds: decoded.duration.toFixed(2),
          preset: presetLabel,
          peakBefore: boosted.peakBefore.toFixed(3),
          peakAfter: boosted.peakAfter.toFixed(3),
    ...
    /**
     * 合成约 2 s、48 kHz 单声道偏薄样例（低音弱、中高偏亮，便于听出 lowshelf）。
     * @returns {File}
     */
    function sampleWav(){
      const rate = 48000;
      ...
      for (let i = 0; i < frames; i++){
        ...
        const thinBass = 0.08 * Math.sin(2 * Math.PI * 80 * t) + 0.06 * Math.sin(2 * Math.PI * 120 * t);
        const body = 0.28 * Math.sin(2 * Math.PI * 440 * t) + 0.18 * Math.sin(2 * Math.PI * 880 * t);
        const air = 0.12 * Math.sin(2 * Math.PI * 2200 * t) + 0.08 * Math.sin(2 * Math.PI * 4400 * t);
        const sample = (thinBass + body + air) * fade;
    ...
    /** 进页或点 Sample：加载偏薄样例并以默认 Medium 自动 Boost。 */
    ...
    $('bassDrop').addEventListener('dragover', event => event.preventDefault());
    $('bassDrop').addEventListener('drop', event => {
    ...
    $('bassConvert').addEventListener('click', convert);
    $('bassSample').addEventListener('click', loadSample);
    $('bassClear').addEventListener('click', () => choose(null));
    $('bassDownload').addEventListener('click', () => {
      if (!outputUrl || busy) return;
      const link = document.createElement('a');
      link.href = outputUrl;
      link.download = (selected.name.replace(/\\.[^.]+$/, '') || 'audio') + '-bass.wav';
	...
	const toolMeta = getToolBySlug('boost-bass-on-an-mp3');
	/** related 等扩展区块。 */
