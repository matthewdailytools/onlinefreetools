  ...
  <style>
    ...
    <div class="tool-panel">
      ...
      <div class="d-flex align-items-center opt-group mb-2 flex-wrap gap-2" id="bcpTargetRow">
        <div class="form-check mb-0">
          <input class="form-check-input" type="checkbox" id="bcpTargetOn" checked>
          <label class="form-check-label" for="bcpTargetOn">${escapeHtml(tx(opts.lang, 'target_on'))}</label>
        </div>
        <label class="form-label mb-0" for="bcpTargetKb">${escapeHtml(tx(opts.lang, 'target_kb_label'))}</label>
        <input type="number" id="bcpTargetKb" class="form-control form-control-sm" style="width:5.5rem;" min="10" max="5000" value="200">
        <span class="small text-muted">KB</span>
      </div>

      <div class="d-flex align-items-center opt-group mb-3 flex-wrap gap-2" id="bcpJpegBgRow">
        <span class="small">${escapeHtml(tx(opts.lang, 'jpeg_bg_label'))}</span>
        <div class="form-check mb-0">
          <input class="form-check-input" type="radio" name="bcpJpegBg" id="bcpBgWhite" value="#ffffff" checked>
          <label class="form-check-label" for="bcpBgWhite">${escapeHtml(tx(opts.lang, 'jpeg_bg_white'))}</label>
        </div>
        <div class="form-check mb-0">
          <input class="form-check-input" type="radio" name="bcpJpegBg" id="bcpBgBlack" value="#000000">
          <label class="form-check-label" for="bcpBgBlack">${escapeHtml(tx(opts.lang, 'jpeg_bg_black'))}</label>
        </div>
        <div class="form-check mb-0">
          <input class="form-check-input" type="radio" name="bcpJpegBg" id="bcpBgCustom" value="custom">
          <label class="form-check-label" for="bcpBgCustom">${escapeHtml(tx(opts.lang, 'jpeg_bg_custom'))}</label>
        </div>
        <input type="color" id="bcpBgColor" value="#ffffff" title="${escapeHtml(tx(opts.lang, 'jpeg_bg_custom'))}">
      </div>
    ...
    (function () {
      ...
      function compressAll() {
        ...
        function next() {
          ...
          var file = queue[i];
          i += 1;
          return compressOne(file, mime, usedNames).then(function (row) {
