/**
 * 批量转换成 PNG 工具页：多图统一 PNG toBlob（不铺垫底），失败跳过，成功打 ZIP。
 * slug: bulk-convert-images-to-png；规格见 work-tasks/bulk-convert-images-to-png/02-tool-info.md。
 */
...
const PREFIX = 'tool_bulk_convert_images_to_png';
 ...
 */
export const renderBulkConvertImagesToPngPage = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/bulk-convert-images-to-png';
	/** 当前语言下的 canonical 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'bulk-convert-images-to-png',
	...
	const extraHeadHtml = `
  <style>
    .bcn-table { font-size: .875rem; }
    .bcn-table td, .bcn-table th { vertical-align: middle; }
  </style>`;

	/** 首屏：多选图、可选最长边（默认关）、汇总表、ZIP。 */
	const contentHtml = `
    ...
    <div class="tool-panel">
      <label class="tool-dropzone mb-3" id="bcnDrop" for="bcnFile">
        <input type="file" id="bcnFile" accept="image/png,image/jpeg,image/webp,image/gif,image/bmp,image/heic,image/heif,image/*" multiple>
        ...
        <span id="bcnFileCount" class="tool-dropzone-file"></span>
      </label>

      <div class="d-flex align-items-center tools-bar mb-2 flex-wrap gap-2">
        <button type="button" id="bcnBtnConvert" class="btn btn-primary btn-sm">${escapeHtml(tx(opts.lang, 'convert'))}</button>
        <button type="button" id="bcnBtnZip" class="btn btn-outline-primary btn-sm" disabled>${escapeHtml(tx(opts.lang, 'download_zip'))}</button>
        <button type="button" id="bcnBtnSample" class="btn btn-outline-secondary btn-sm">${escapeHtml(tx(opts.lang, 'sample'))}</button>
        <button type="button" id="bcnBtnClear" class="btn btn-outline-secondary btn-sm">${escapeHtml(tx(opts.lang, 'clear'))}</button>
      </div>

      <div class="d-flex align-items-center opt-group mb-3 flex-wrap gap-2">
        <div class="form-check mb-0">
          <input class="form-check-input" type="checkbox" id="bcnResizeOn">
          <label class="form-check-label" for="bcnResizeOn">${escapeHtml(tx(opts.lang, 'resize_on'))}</label>
        </div>
        <label class="form-label mb-0" for="bcnMaxEdge">${escapeHtml(tx(opts.lang, 'max_edge_label'))}</label>
        <input type="number" id="bcnMaxEdge" class="form-control form-control-sm" style="width:5.5rem;" min="64" max="8192" value="2048">
      </div>

      <p id="bcnWarn" class="alert alert-warning py-2 small mb-2" style="display:none;" role="status"></p>
      <p id="bcnError" class="alert alert-danger py-2 small mb-2" style="display:none;" role="alert"></p>
      <p id="bcnStatus" class="small text-muted mb-2" role="status"></p>
      <p id="bcnSummary" class="small mb-2"></p>

      <div class="table-responsive mb-0">
        <table class="table table-sm table-striped bcn-table mb-0">
          <thead>
            <tr>
	...
	/**
	 * 客户端脚本：串行 PNG 编码（不铺垫底）、体积变大标记、失败跳过、ZIP。
  ...
  <script>
    (function () {
      ...
      /** 拖放区。 */
      var drop = document.getElementById('bcnDrop');
      /** 多文件选择。 */
      var fileInput = document.getElementById('bcnFile');
      /** 队列计数。 */
      var fileCountEl = document.getElementById('bcnFileCount');
      /** 是否限制最长边（默认关，避免转换时偷偷缩放）。 */
      var resizeOn = document.getElementById('bcnResizeOn');
      /** 最长边像素。 */
      var maxEdgeEl = document.getElementById('bcnMaxEdge');
      /** 转换全部。 */
      var btnConvert = document.getElementById('bcnBtnConvert');
      /** 下载 ZIP。 */
      var btnZip = document.getElementById('bcnBtnZip');
      /** 样例。 */
      var btnSample = document.getElementById('bcnBtnSample');
      /** 清空。 */
      var btnClear = document.getElementById('bcnBtnClear');
      /** 警告条。 */
      var warnEl = document.getElementById('bcnWarn');
      /** 错误条。 */
      var errEl = document.getElementById('bcnError');
      /** 状态条。 */
      var statusEl = document.getElementById('bcnStatus');
      /** 汇总。 */
      var summaryEl = document.getElementById('bcnSummary');
      /** 表体。 */
      var tbody = document.getElementById('bcnTbody');
      ...
      var msg = {
        ...
        larger: ${JSON.stringify(tx(opts.lang, 'warn_larger'))},
        ...
        grew: ${JSON.stringify(tx(opts.lang, 'status_larger'))},
        ...
        function next() {
          if (i >= queue.length) {
            summaryEl.textContent = msg.summaryTpl
              ...
              .replace('{larger}', String(larger))
              .replace('{skip}', String(skip));
            setStatus(msg.done);
            if (larger > 0) setWarn(msg.larger);
          ...
          return convertOne(file, usedNames).then(function (row) {
            if (row.kind === 'ok') ok += 1;
            else if (row.kind === 'larger') larger += 1;
            else skip += 1;
            if (row.bytes && row.zipName) zipParts.push({ zipName: row.zipName, bytes: row.bytes });
            var statusLabel = row.kind === 'ok' ? msg.ok : row.kind === 'larger' ? msg.grew : msg.skip;
            appendRow({
              name: row.name,
      ...
      function downloadZip() {
        ...
        try {
          ...
          var url = URL.createObjectURL(blob);
          a.href = url;
          a.download = 'images-converted-to-png.zip';
	...
	/** catalog 元数据。 */
	const toolMeta = getToolBySlug('bulk-convert-images-to-png');
	/** FAQ / related 区块。 */
