/**
 * 在线编辑 PDF 文本：pdf.js 抽出文本层 → 文本框改写 → pdf-lib drawText 简单重排（非原地改字形）。
...
export const renderEditPdfTextOnlinePage = (opts: {
  ...
  <style>
	...
	/**
	 * 客户端脚本：抽出文本层到编辑器，Apply 时用 drawText 简单重排新页。
	 * 不是原地改字形；扫描件无文本层会失败。
	 */
	const extraBodyHtml = `
  ${pdfWorkUiClientScript()}
  ...
  <script>
    (function () {
      ...
      var source = null;
      /** @type {Uint8Array|null} 重排后的 PDF 字节 */
      ...
      /**
       * 把编辑器全文按页简单重排为新 PDF（Helvetica drawText）。
       * @param {string} raw
      ...
      function loadFile(file) {
        ...
        return file.arrayBuffer().then(function (buf) {
          ...
          return ensurePdfJs().then(function (pdfjsLib) {
            ...
            refreshMeta();
            work.setProgress(40);
            return buildEditorText(bytes, function (done, total) {
      ...
      function applyEdit() {
        ...
        if (!String(editorEl.value || '').trim()) {
          setError(source ? msg.noText : msg.empty);
          return;
        }
        if (!hasPdfLib()) {
      ...
      /**
       * 进页/按钮样例：载入样例 PDF、填入编辑器、再 Apply 一次。
      ...
      function loadSample() {
        ...
        return makeSamplePdf().then(function (file) {
          ...
          if (String(editorEl.value || '').trim()) applyEdit();
        });
      }
