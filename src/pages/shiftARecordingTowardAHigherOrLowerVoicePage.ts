...
const P = 'tool_shift_a_recording_toward_a_higher_or_lower_voice';
...
/**
 * 把录音移向更高或更低人声：按半音重采样再 WSOLA 回正时长，导出 16-bit WAV（S39）。
 * ≠ 性别转换承诺；≠ S38 玩具变声；≠ S33 歌曲半音场景。
 * @param opts.lang 当前 UI 语言
 * @param opts.defaultLang 默认（无前缀）语言
 * @param opts.enabledLangs 启用语言列表
 */
export const renderShiftARecordingTowardAHigherOrLowerVoicePage = (opts: {
	lang: SiteLang;
	defaultLang: SiteLang;
	enabledLangs: SiteLang[];
}) => {
	/** 工具规范路径（无语言前缀）。 */
	const toolPath = '/tools/shift-a-recording-toward-a-higher-or-lower-voice';
	/** 当前语言的规范 URL 路径。 */
	...
	const sidebarHtml = renderSidebar({
		...
		groups: buildToolSidebarItems({
			...
			currentSlug: 'shift-a-recording-toward-a-higher-or-lower-voice',
			currentAnchor: '#voice-height',
		}),
		id: 'toolNav',
	});
	...
	/** 工具主面板与结果区 markup。 */
	const contentHtml = `
    <div id="voice-height" class="tool-page-heading mb-3"><h1 class="h4">${tr('title')}</h1><p>${tr('desc')}</p></div>
    <section class="tool-panel" id="vhPanel">
      <label class="tool-dropzone mb-3" id="vhDrop" for="vhFile"><input id="vhFile" type="file" accept=".wav,.mp3,.m4a,.aac,.ogg,audio/*"><span class="tool-dropzone-title">${tr('choose')}</span><span class="tool-dropzone-hint">${tr('hint')}</span><span id="vhName" class="tool-dropzone-file"></span></label>
      <div class="tools-bar d-flex flex-wrap mb-3" style="gap:.5rem">
        <button id="vhConvert" class="btn btn-primary" type="button">${tr('convert')}</button>
        <button id="vhDownload" class="btn btn-outline-primary" type="button" disabled>${tr('download')}</button>
        <button id="vhSample" class="btn btn-outline-secondary" type="button">${tr('sample')}</button>
        <button id="vhClear" class="btn btn-outline-secondary" type="button">${tr('clear')}</button>
      </div>
      <details class="mb-3" open><summary>${tr('advanced')}</summary>
        <p class="form-label mt-2 mb-1">${tr('voice_label')}</p>
        <div class="vh-preset-chips mb-2" id="vhPresetChips" role="group" aria-label="${tr('voice_label')}">
          <button type="button" class="btn btn-sm btn-primary" data-preset="higher" aria-pressed="true">${tr('preset_higher')}</button>
          <button type="button" class="btn btn-sm btn-outline-secondary" data-preset="much_higher" aria-pressed="false">${tr('preset_much_higher')}</button>
          <button type="button" class="btn btn-sm btn-outline-secondary" data-preset="lower" aria-pressed="false">${tr('preset_lower')}</button>
          <button type="button" class="btn btn-sm btn-outline-secondary" data-preset="much_lower" aria-pressed="false">${tr('preset_much_lower')}</button>
        </div>
        <p class="form-text">${tr('settings_hint')}</p>
      </details>
      <div id="vhHud" class="bcw-hud mb-3" role="status" aria-live="polite" hidden>
        <div class="bcw-hud-top"><div class="bcw-hud-spin" aria-hidden="true"></div><div class="bcw-hud-pct" id="vhPct">0%</div><div class="bcw-hud-copy"><div class="bcw-hud-title">${tr('progress')}</div><div class="bcw-hud-step" id="vhStep"></div><div class="bcw-hud-time" id="vhTime"></div></div></div>
        <div class="progress" style="height:1.35rem"><div id="vhBar" class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div><span class="bcw-hud-sheen" aria-hidden="true"></span></div>
        <ol class="bcw-hud-steps"><li data-step="read">${tr('read')}</li><li data-step="decode">${tr('decode')}</li><li data-step="shift">${tr('shift')}</li><li data-step="write">${tr('write')}</li></ol><div class="bcw-hud-url" id="vhCurrent"></div>
      </div>
      <div id="vhOutput" hidden><h2 class="h5">${tr('preview')}</h2><p id="vhResult"></p><audio id="vhAudio" controls preload="metadata" style="max-width:100%" aria-label="${tr('preview')}"></audio></div>
    </section>`;
	...
	/** 金标 HUD、预设芯片与布局补充样式。 */
	const extraHeadHtml = `<style>${bcwHudCss({ hudId: 'vhHud', convertBtnId: 'vhConvert' })}#vhHud.is-error{border-color:#b91c1c;background:#fff1f2}#vhHud:not(.is-on) .bcw-hud-spin{animation:none}.vh-preset-chips{display:flex;flex-wrap:wrap;gap:.4rem}.site-footer nav.d-inline{display:inline-flex!important;flex-wrap:wrap;justify-content:center;max-width:100%}</style>`;
  ...
  (function(){
    ...
    /** 文件输入。 */
    const fileInput = $('vhFile');
    /** 人声高度芯片容器。 */
    const presetChips = $('vhPresetChips');
    /** 进度 HUD。 */
    const hud = $('vhHud');
    /** 预览 audio。 */
    const audio = $('vhAudio');
    /** HUD 步骤顺序。 */
    const STEPS = ['read','decode','shift','write'];
    /** 峰值软保护上限。 */
    const PEAK_LIMIT = 0.98;
    /** 预设键 → 半音偏移。 */
    const PRESET_ST = { higher: 4, much_higher: 7, lower: -4, much_lower: -7 };
    /** 合法预设键列表。 */
    const PRESET_KEYS = ['higher','much_higher','lower','much_lower'];
    /** 当前选中文件。 */
    let selected = null;
    /** 输出 object URL。 */
    let outputUrl = null;
    ...
    function discard(){
      ...
      audio.load();
      $('vhOutput').hidden = true;
      $('vhResult').textContent = '';
      $('vhDownload').disabled = true;
    }
    /**
     * 锁定或解锁控件。
    ...
    function lock(on){
      busy = on;
      $('vhConvert').disabled = on;
      $('vhSample').disabled = on;
      $('vhClear').disabled = on;
      fileInput.disabled = on;
      presetChips.querySelectorAll('button').forEach(btn => { btn.disabled = on; });
      if (!on) $('vhDownload').disabled = !outputUrl;
    }
    /**
     * 刷新人声高度芯片选中态。
    ...
    function resolveSemitones(){
      if (PRESET_KEYS.indexOf(preset) < 0) throw Error('err_preset');
      const v = PRESET_ST[preset];
      if (!Number.isFinite(v)) throw Error('err_preset');
      return v;
    }
    /**
     * 当前预设的展示标签。
    ...
    function presetLabel(){
      return M['preset_' + preset] || preset;
    }
    /**
     * 更新 HUD 百分比与步骤高亮。
    ...
    function progress(pct, stepKey){
      ...
      $('vhPct').textContent = value + '%';
      $('vhBar').style.width = value + '%';
      $('vhBar').setAttribute('aria-valuenow', String(value));
      const label = stepKey === 'done' ? M.done : (M[stepKey] || stepKey);
      $('vhStep').textContent = label;
    ...
    function fail(key){
      ...
      $('vhPct').textContent = '—';
      $('vhBar').style.width = '0%';
      $('vhBar').setAttribute('aria-valuenow', '0');
      hud.setAttribute('role', 'alert');
      $('vhStep').textContent = M[key] || M.failed;
    }
    /**
     * 选择或清空输入文件（会丢弃旧输出）。
    ...
    function choose(file){
      ...
      discard();
      selected = file || null;
      $('vhName').textContent = file ? file.name : '';
      hud.hidden = true;
      fileInput.value = '';
    }
    /**
    ...
    async function convert(){
      ...
      $('vhCurrent').textContent = selected.name;
      started = performance.now();
      const clock = () => { $('vhTime').textContent = fill(M.elapsed, { s: ((performance.now() - started) / 1000).toFixed(1) }); };
      clock();
      ...
      try {
        ...
        audio.src = outputUrl;
        $('vhOutput').hidden = false;
        $('vhResult').textContent = fill(M.result, {
    ...
    /**
     * 合成约 3 s、48 kHz 单声道含稳定基频的样例，便于听出人声高度变调。
     * @returns {File}
     */
    function sampleWav(){
      const rate = 48000;
    ...
    /** 进页或点 Sample：加载样例并以默认 Higher 自动 Shift。 */
    ...
    $('vhDrop').addEventListener('dragover', event => event.preventDefault());
    $('vhDrop').addEventListener('drop', event => {
    ...
    $('vhConvert').addEventListener('click', convert);
    $('vhSample').addEventListener('click', loadSample);
    $('vhClear').addEventListener('click', () => choose(null));
    $('vhDownload').addEventListener('click', () => {
      if (!outputUrl || busy) return;
      const link = document.createElement('a');
      link.href = outputUrl;
      link.download = (selected.name.replace(/\\.[^.]+$/, '') || 'audio') + '-voice-height.wav';
	...
	const toolMeta = getToolBySlug('shift-a-recording-toward-a-higher-or-lower-voice');
	/** related 等扩展区块。 */
