...
const en: SiteLangDict = {
  ...
  tool_color_from_image_article:
    'Local dominant-color palette plus click-to-pick region average with copyable HEX/RGB/HSL.',
  ...
  tool_color_from_image_faq_a2: 'The page shrinks the image, drops low-alpha pixels, splits color space with median-cut, averages each bucket, and sorts by how many samples fell in each bucket.',
  tool_color_from_image_faq_q3: 'What happens when I click the image?',
  tool_color_from_image_faq_a3: 'The tool maps the click back to source pixels and averages opaque colors in a small square (~7×7). You get HEX/RGB/HSL for that region, separate from the palette list.',
  tool_color_from_image_faq_q4: 'Why is a click not a single pixel?',
  tool_color_from_image_faq_a4: 'Photos and UI shots are noisy at 1×1. A tiny region average is more stable for CSS accents while still tracking where you clicked.',
  ...
  tool_image_merge_article: 'Local multi-image collage with layout, gap, background, grid fit, output format, and optional size presets.',
  ...
  tool_image_merge_rules_body: 'Canvas size follows the formulas below. Grid cells share one width and height; contain letterboxes, cover crops.',
  ...
  tool_image_merge_faq_a2: 'Horizontal unifies height and grows width; vertical unifies width and grows height. Grid uses fixed cells with contain or cover.',
  ...
  tool_image_merge_faq_a5: 'Very large canvas edges (above ~8192 px) or many megapixel sources can exceed browser memory. Reduce count, use grid with fewer columns, or resize sources first with the crop tool.',
  ...
  tool_image_overlay_article: 'Local two-layer compositing with opacity, top-left position, scale, blend modes, and JPEG matte when flattening transparency.',
  ...
  tool_image_overlay_rules_item_1:
  ...
  tool_image_overlay_faq_a1: 'No. Decode, composite, and export run in your browser. Check the Network panel — files are not POSTed.',
  tool_image_overlay_faq_q2: 'How is overlay different from merge?',
  tool_image_overlay_faq_a2: 'Merge stitches several photos into one layout (horizontal, vertical, or grid). Overlay keeps one base and paints one layer on top with opacity, position, and blend.',
  tool_image_overlay_faq_q3: 'What do the blend modes do?',
  tool_image_overlay_faq_a3: 'They follow Canvas composite rules: Normal stacks the layer; Multiply darkens; Screen lightens; Overlay boosts contrast; Darken/Lighten pick min/max channels. Preview updates live.',
  tool_image_overlay_faq_q4: 'Why pick a JPEG background color?',
  tool_image_overlay_faq_a4: 'JPEG has no transparency. Before export the tool fills the canvas with your chosen color so transparent PNG areas become solid instead of turning black.',
