...
const ja: SiteLangDict = {
  ...
  tool_batch_convert_web_pages_to_jpg_desc: 'URL リストのウェブページをまとめて JPG にし ZIP で渡す。取得は一度きりで保存せず、変換は端末内。',
  ...
  tool_batch_convert_web_pages_to_pdf_article: '公開ページの URL を1行ずつ貼り、このタブで A4 の PDF に変換し、失敗した行は飛ばして ZIP にまとめます。各アドレスはサーバー経由で一度だけ取得し保存しません。変換は端末内です。',
  ...
  tool_batch_convert_web_pages_to_pdf_desc: 'URL リストのウェブページをまとめて PDF にし ZIP で渡す。取得は一度きりで保存せず、変換は端末内。',
  ...
  tool_batch_convert_web_pages_to_pdf_faq_a1: '変換はこのタブで行います。URL リストを変換するとき、各アドレスをサーバーへ一度送って HTML を取りますが、ページは保存しません。PDF は端末で作り、ここで ZIP にまとめます。',
  tool_batch_convert_web_pages_to_pdf_faq_a2: 'こちらは URL リスト用（複数ページを PDF にして ZIP）です。HTML 断片やプレビュー付きの1件変換は「HTML を PDF に変換」を使ってください。',
  tool_batch_convert_web_pages_to_pdf_faq_a3: 'なりません。スクリプトは削除します。ログイン壁・有料壁・JavaScript 描画のアプリは実サイトと一致しないことが多いです。印刷用 CSS も違う場合があります。',
  tool_batch_convert_web_pages_to_pdf_faq_a4: '成功した行は別々の PDF として1つの ZIP に入ります。1ファイルへの結合はしません。必要ならダウンロード後に「ファイルを1つの PDF に結合」を使ってください。',
  tool_batch_convert_web_pages_to_pdf_faq_a5: 'http(s) URL は最大10件。それ以上は無視します。失敗した行は飛ばし、残りは続けます。',
  ...
  tool_batch_convert_web_pages_to_pdf_how_body: '公開ページの URL リストを貼り、順に PDF へ変換し、できたファイルを ZIP で持ち帰りましょう。',
  ...
  tool_batch_convert_web_pages_to_pdf_rules_body: 'http(s) URL は1行1件、最大10。直列変換。失敗はスキップ。成功した PDF は ZIP へ。',
  tool_batch_convert_web_pages_to_pdf_rules_item_1: '各 URL は HTML 取得のためサーバーへ一度送り、保存しません。PDF はこのタブで作ります。',
  tool_batch_convert_web_pages_to_pdf_rules_item_2: 'プライベート網・ログイン・存在しないページはその行だけ失敗し、残りは続きます。',
  tool_batch_convert_web_pages_to_pdf_rules_item_3: 'ZIP 内の同名は name (2).pdf になります。取り込み前にスクリプトを外します。',
  tool_batch_convert_web_pages_to_pdf_rules_item_4: 'サイト全体のクローラーではありません。手元の URL を貼ってください。',
  ...
  tool_batch_convert_web_pages_to_pdf_url_hint: '公開 http(s) URL を1行1件、最大10。各ページを一度取得し CSS と画像 URL を書き換えます。PDF 前にスクリプトを削除します。',
  ...
  tool_batch_convert_web_pages_to_pdf_wait_hint: '画像の読み込み開始後に待つ秒数です。初期値は1。PDF に空の画像枠が出るときは増やしてください。',
