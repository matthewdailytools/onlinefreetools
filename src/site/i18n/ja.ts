...
const ja: SiteLangDict = {
  ...
  tool_convert_html_to_pdf_description:
    'HTML を PDF にするには、断片を貼るかページ URL を読み込み、スクリプトを除いてプレビューし、A4 で保存します。URL 読み込みはサーバーが HTML を一度だけ取得し、保存しません。変換はこのタブ内です。ログインが必要なページや JS で描画するサイト、印刷用 CSS は実ページと違うことがあります。',
  ...
  tool_make_pdf_flipbook_description:
    'ブラウザで PDF めくり冊子を作ります。PDF を選び、短いページめくりで確認し、各ページを画像として埋め込んだ単体 HTML を保存します。出力はオフラインで開け、文書は送信しません。サンプルは A4 の 2 ページで、保存後も同じページサイズです。',
