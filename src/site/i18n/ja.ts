...
const ja: SiteLangDict = {
  ...
  tool_edit_pdf_text_online_description:
    'ブラウザでPDFのテキストをオンライン編集します。テキスト層を取り出し、編集欄で直し、適用で単純に再描画します。グリフの直接編集ではなく、元のフォントや段組は残りません。ファイルは端末内に留まり、サーバーにアップロードしません。入場時のサンプルでプロセスと手順の例が動きます。スキャン頁は失敗します。',
  ...
  tool_edit_pdf_text_online_faq_a4:
    '「テキスト追加」は X/Y に新行を重ねます。「PDF を編集可能に」は主に編集可能な .txt をダウンロード（任意で再構築 PDF）。本ページの主ダウンロードはテキスト層編集後の再構築 .pdf です。',
  ...
  tool_make_pdf_flipbook_description:
    'PDF フリップブックを作成. ファイルは端末内に留まり、サーバーにアップロードしません。 サンプルで実際の入力と出力を確認でき、手順と制限は本文で説明します。 手順は PDF を選び、前へ・次へで短いページめくり表示を確認し、PDF を内蔵した単体 HTML を保存します。例は1ページの flipbook です。出力 HTML は開くときに PDF.js を本サイトから読むためネット接続が必要ですが、文書は当サイトへ送信しません。',
