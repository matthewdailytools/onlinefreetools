...
const ru: SiteLangDict = {
  ...
  tool_color_from_image_article:
    'Локальная палитра доминирующих цветов плюс среднее по области при клике, HEX/RGB/HSL копируются.',
  ...
  tool_color_from_image_pick_hint: 'Кликните по изображению, чтобы усреднить небольшую область вокруг точки (около 7×7 исходных пикселей).',
  ...
  tool_color_from_image_faq_a2: 'Страница уменьшает изображение, отбрасывает низкоальфа-пиксели, делит цветовое пространство median-cut, усредняет группы и сортирует по числу выборок.',
  tool_color_from_image_faq_q3: 'Что происходит при клике по изображению?',
  tool_color_from_image_faq_a3: 'Инструмент переводит клик в исходные пиксели и усредняет непрозрачные цвета в небольшом квадрате (~7×7). Вы получаете HEX/RGB/HSL для этой области, отдельно от списка палитры.',
  tool_color_from_image_faq_q4: 'Почему клик — не один пиксель?',
  tool_color_from_image_faq_a4: 'Фото и UI-скриншоты шумны на 1×1. Среднее малой области стабильнее для CSS-акцентов и всё же отражает место клика.',
  ...
  tool_image_merge_article: 'Локальный коллаж из нескольких изображений: раскладка, отступ, фон, fit в сетке, формат и пресеты размера.',
  ...
  tool_image_merge_rules_body: 'Размер холста по формулам ниже. В сетке ячейки одной ширины и высоты; «вместить» даёт поля, «заполнить» обрезает.',
  ...
  tool_image_merge_faq_a2: 'Горизонт выравнивает высоту и растёт в ширину; вертикаль — ширину и растёт в высоту. Сетка — фиксированные ячейки с вместить/заполнить.',
  ...
  tool_image_merge_faq_a5: 'Стороны выше ~8192 px или слишком много мегапикселей могут превысить память браузера. Уменьшите число файлов, столбцы сетки или обрежьте исходники.',
  ...
  tool_image_overlay_article: 'Локальная двухслойная композиция: прозрачность, якорь сверху слева, масштаб, режимы, фон JPEG при сглаживании alpha.',
  ...
  tool_image_overlay_rules_body: 'Холст вывода = натуральные пиксели базы. Слой рисуется с якорем сверху слева в (x, y) после масштаба.',
  tool_image_overlay_rules_item_1: 'Ширина × высота холста = размер базы; база рисуется в (0, 0).',
  tool_image_overlay_rules_item_2: 'Размер слоя = натуральная ширина × высота × (масштаб% ÷ 100); позиция в пикселях базы, якорь сверху слева.',
  ...
  tool_image_overlay_faq_a2: 'Склейка собирает несколько фото в ряд, колонку или сетку. Наложение оставляет одну базу и рисует слой с прозрачностью, позицией и режимом.',
  tool_image_overlay_faq_q3: 'Что делают режимы наложения?',
  tool_image_overlay_faq_a3: 'Правила Canvas: Обычный кладёт сверху; Multiply темнее; Screen светлее; Overlay контрастнее; Darken/Lighten — min/max каналов.',
  tool_image_overlay_faq_q4: 'Зачем цвет фона JPEG?',
  tool_image_overlay_faq_a4: 'У JPEG нет прозрачности. Перед экспортом холст заливается, чтобы прозрачные PNG-зоны не стали чёрными.',
  tool_image_overlay_faq_q5: 'Текстовый водяной знак здесь?',
  tool_image_overlay_faq_a5: 'Страница накладывает только файлы изображений. Для текста используйте инструмент текстового водяного знака.',
...
tool_ip_address_title: 'Узнать свой IP — Публичный адрес (IPv4/IPv6)',
  ...
  tool_ip_address_rules_item_5: 'Регион и ISP из метаданных IP edge — часто уровень города; mobile/CGNAT может ошибаться; VPN показывает узел выхода.',
  ...
  tool_ip_address_faq_a5:
    'Запрос короткий на edge. Истории IP нет; считайте результат временным отображением.',
