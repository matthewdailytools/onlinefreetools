...
const ja: SiteLangDict = {
	...
	tool_chatgpt_export_to_markdown_faq_a3: 'はい。Claude の chat_messages 配列（sender + text）も同じコンバーターが解析するので、一度貼り付けるだけで ChatGPT と Claude 両方のエクスポートを扱えます。',
	tool_chatgpt_export_to_markdown_faq_a4: '空の system と本文のない tool 殻は飛ばします。添付や画像はファイルとしては復元しません。',
	tool_chatgpt_export_to_markdown_faq_a5: 'トークン集計はこの変換器の仕事ではありません。必要なときは専用のトークンカウンタを使ってください。',
	...
	tool_chatgpt_export_to_markdown_how_item_4: 'コピーまたは保存。デモを入れるときは「サンプル読込」をクリック。',
	...
	tool_chatgpt_export_to_markdown_rules_body:
		'エクスポート変換にはフィールド対応表、既定の Markdown、端末内処理のプライバシー説明が必要です。',
	...
	tool_chatgpt_export_to_markdown_rules_item_3: 'JSON / CSV チップは同じターンを別形式で出し直し、パイプラインや表計算向けです。',
	...
	tool_chatgpt_export_to_markdown_usecase_3: 'Claude の chat_messages JSON もこのページにドロップすれば、同じ変換器がターンを Markdown にします。',
