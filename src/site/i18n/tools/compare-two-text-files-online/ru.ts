...
const ru: SiteLangDict = {
	...
	tool_compare_two_text_files_online_description:
		'Сравнить два текстовых файла онлайн в браузере. Процесс: выберите исходный файл и правленый, декодируйте как UTF-8 (BOM снимается), двоичные байты отклоняются, затем подсвечиваются добавления и удаления строк. Файлы остаются на вашем устройстве и не загружаются. Пример: notes-a.txt и notes-b.txt, меняется только средняя строка. Word, Excel и JSON — на других страницах; здесь только текстовые файлы. Сравнить txt — та же задача.',
	...
	tool_compare_two_text_files_online_faq_a2:
		'Декодирование UTF-8. Начальный BOM снимается, чтобы первая строка не считалась изменённой только из‑за этой метки. Другие кодировки могут «поехать» — сначала сохраните как UTF-8.',
	...
	tool_compare_two_text_files_online_faq_a4:
		'Страница для .txt / .text. Поиск «сравнить txt» или онлайн-diff файлов обычно про это. Если «сравнить два файла» значило Word или таблицу — идите на те страницы. Вставка двух текстов без файлов — «Проверить различия между двумя текстами».',
	...
	tool_compare_two_text_files_online_how_item_3:
		'Сравнение запускается само, когда оба файла выбраны; «Загрузить пример» уже сделал это при первой отрисовке.',
