...
const ja: SiteLangDict = {
	...
	tool_convert_html_web_pages_to_word_document_how_item_3:
		'HTML 内の <img> も入れたい場合は詳細設定を開きます。埋め込みは既定オンで、ブロックされた画像はスキップします。見出しスタイルも表も要らず文字だけなら「プレーンテキスト」をオン（画像もオフになります）。',
	...
	tool_convert_html_web_pages_to_word_document_hud_download: 'Word ファイルをダウンロード',
	tool_convert_html_web_pages_to_word_document_hud_elapsed_tpl: '{s}秒経過',
	tool_convert_html_web_pages_to_word_document_hud_fail_hint: 'HTML か URL を確かめてから、もう一度「変換」してください。',
	tool_convert_html_web_pages_to_word_document_hud_fail_title: '変換に失敗しました',
	tool_convert_html_web_pages_to_word_document_hud_next: '変換が終わりました。次は .docx をダウンロードして開き、文字を直します。',
	tool_convert_html_web_pages_to_word_document_hud_pct_tpl: '{pct}%',
	tool_convert_html_web_pages_to_word_document_hud_step_fetch: '取得',
	tool_convert_html_web_pages_to_word_document_hud_step_map: '対応づけ',
	tool_convert_html_web_pages_to_word_document_hud_step_pack: '梱包',
	tool_convert_html_web_pages_to_word_document_hud_title: '変換の進み',
	...
	tool_convert_html_web_pages_to_word_document_opts_hint:
		'「画像を含める」は HTML の絵を埋め込もうとします。「プレーンテキスト」は見出し・表・リンクを普通の段落にします。CSS の配置はマップしません。出力は常に1つの .docx です。',
	tool_convert_html_web_pages_to_word_document_plain_hint:
		'編集できる段落だけ残します。Word の見出しスタイル、表オブジェクト、埋め込み画像は使いません。リンクは「文言 (URL)」、表のセルはタブ区切りの行になります。',
	tool_convert_html_web_pages_to_word_document_plain_label: 'プレーンテキスト',
	...
	tool_convert_html_web_pages_to_word_document_rules_item_2:
		'既定では h1–h6 は Word 見出し、ul/ol はリスト、table は表、a はハイパーリンク。詳細設定の「プレーンテキスト」を入れると、どれも通常の段落になります。',
	...
	tool_convert_html_web_pages_to_word_document_status_done: '完了です。次は「ダウンロード」です。',
	tool_convert_html_web_pages_to_word_document_status_fetching: 'URL の HTML を取得しています…',
	tool_convert_html_web_pages_to_word_document_status_mapping: 'HTML を Word の段落に対応づけています…',
	tool_convert_html_web_pages_to_word_document_status_packing: '.docx を梱包しています…',
