/**
 * i18n tool shard (film-prompt-builder / zh).
...
const zh: SiteLangDict = {
	tool_film_prompt_builder_article:
		'在本页构建可粘贴的 电影 Prompt 构建器 Prompt。填写字段，复制 Markdown 或 JSON 到 ChatGPT、Gemini、Claude 或 DeepSeek。默认本地组装；可选 Expand/Polish 使用 Cloudflare Workers AI（Turnstile）。不用 AI 时文本留在本机。',
	...
	tool_film_prompt_builder_desc:
		'电影 Prompt 构建器 — 默认本地组装，可选 Cloudflare AI 扩写/润色（Turnstile）；Markdown/JSON 留在本机。',
	...
	tool_film_prompt_builder_faq_a3:
		'Prompt 模板构建器 covers generic Role/Task/Constraints/Output templates. This page focuses on film prompt builder fields with a dedicated sample and rules for this scenario.',
	...
	tool_film_prompt_builder_how_body:
		'填写 电影 Prompt 构建器 字段，本地构建 Prompt，可选 Turnstile 后 Expand/Polish，再粘贴到 ChatGPT、Gemini、Claude 或 DeepSeek。',
	...
	tool_film_prompt_builder_rules_body:
		'电影 Prompt 构建器：默认本地组装；可选 Cloudflare AI 有频率限制且须 Turnstile。',
	...
	tool_film_prompt_builder_title:
		'电影 Prompt 构建器 — 本地 + 可选 AI',
	tool_film_prompt_builder_usecase_1:
		'为团队在 ChatGPT、Gemini、Claude 或 DeepSeek 准备可粘贴的 电影 Prompt 构建器 简报。',
