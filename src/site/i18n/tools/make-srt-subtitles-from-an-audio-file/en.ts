...
const en: SiteLangDict = {
  ...
  tool_make_srt_subtitles_from_an_audio_file_description: 'Make SRT subtitles from an audio file in the browser using browser speech recognition when your browser provides it. Steps: Choose a local audio file, pick a recognition language, Make SRT while playing (speakers + mic loopback) or Dictate with mic, edit the SRT preview, download .srt. Example: Load sample opens a short tone and runs the playthrough path—real speech needs a voice recording or the mic button. Cue times are best-effort from recognition timing, not frame-perfect forced alignment. This is not a cloud a large speech model clone and does not load a huge offline model ASR model. Many browsers cannot reliably subtitle uploaded files offline. The page does not upload your file for server-side ASR.',
  ...
  tool_make_srt_subtitles_from_an_audio_file_usecase_3: 'You need a starter SRT from speech recognition to edit before publishing video.',
  tool_make_srt_subtitles_from_an_audio_file_faq_q1: 'Is this a large speech model clone or cloud ASR upload?',
  tool_make_srt_subtitles_from_an_audio_file_faq_a1: 'No. It does not ship a large offline speech model model and does not upload your file to our servers for ASR. It uses the browser speech recognition API when available. Vendor speech services may still apply to microphone audio—see browser privacy docs.',
  tool_make_srt_subtitles_from_an_audio_file_faq_q2: 'How accurate are the SRT timestamps?',
  tool_make_srt_subtitles_from_an_audio_file_faq_a2: 'They are best-effort estimates from when final recognition results arrive (the browser audio engine clock while playing, or session elapsed time for mic)—not frame-perfect forced alignment like desktop a large speech model pipelines.',
