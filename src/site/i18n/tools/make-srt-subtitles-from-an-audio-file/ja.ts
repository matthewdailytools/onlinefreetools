...
const ja: SiteLangDict = {
  ...
  tool_make_srt_subtitles_from_an_audio_file_description: 'ブラウザの browser speech recognition がある場合、ローカル音声から SRT 字幕を作れます。手順：音声を選ぶ、言語を選ぶ、再生しながら SRT またはマイク口述、編集、.srt を保存。例：サンプル読込は短いトーンで再生パスを実行—実音声ファイルかマイクが必要。キュー時刻は最終結果到着時の推定で、a large speech model 級の強制整列ではありません。a large speech model クローンではなく巨大 offline model も読み込みません。多くのブラウザはアップロードファイルの確実なオフライン字幕化を持ちません。このページはサーバー ASR のためにファイルをアップロードしません。',
  ...
  tool_make_srt_subtitles_from_an_audio_file_why_choose_title: 'この音声ファイルから SRT 字幕を作るツールを選ぶ理由',
  ...
  tool_make_srt_subtitles_from_an_audio_file_usecase_3: '公開前に直すための、音声認識ベースの初期SRTが欲しい。',
  tool_make_srt_subtitles_from_an_audio_file_faq_q1: 'a large speech model クローンやクラウド ASR アップロードですか？',
  tool_make_srt_subtitles_from_an_audio_file_faq_a1: 'いいえ。a large offline speech model は同梱せず、ASR のために当サーバーへアップロードもしません。利用可能な場合はブラウザの browser speech recognition を使います。',
