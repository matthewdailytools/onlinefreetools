...
const ru: SiteLangDict = {
	...
	tool_onpage_description:
		'Проверьте title, meta description, H1, canonical, robots, Open Graph и JSON-LD. Вставьте URL или HTML — код остаётся в браузере, на сервер не уходит. Пример: два H1.',
	...
	tool_onpage_desc_missing: 'Meta description не найден. Тогда Google соберёт сниппет из текста страницы. Пишите своё резюме, если хотите управлять этой формулировкой.',
	...
	tool_onpage_h1_missing: 'H1 не найден. Добавьте заголовок верхнего уровня с темой страницы: на него опираются и скринридеры, и заголовок сниппета.',
	tool_onpage_h1_multiple: 'Найдено {n} тегов H1. Google ранжирует страницы с любым числом H1, так что это не штраф. И всё же один H1 с разделами на H2–H6 понятнее звучит в скринридере.',
	...
	tool_onpage_canonical_other: 'Canonical указывает на другой URL — так вы сообщаете Google, что эта страница дублирует ту. Оставляйте так, только если это правда: canonical — подсказка, поэтому Google всё равно может выбрать другой URL.',
	...
	tool_onpage_how_item_1: 'Оставьте «Вставить HTML» или переключитесь на «Загрузить URL» для живой страницы.',
	tool_onpage_how_item_2: 'Вставьте разметку или введите https URL.',
	tool_onpage_how_item_3: 'Нажмите «Проверить страницу» (или «Пример» для демо).',
	tool_onpage_how_item_4: 'Просмотрите статус каждой проверки и исправьте указанные теги.',
	tool_onpage_rules_title: 'Правила проверок',
	tool_onpage_rules_body:
		'С чем сверяет каждая проверка и насколько это строго. Поведение тегов (robots, canonical, смешанный контент) описано в Google Search Central и спецификации HTML. А советы про длину и заголовки — эвристики отображения и читаемости, а не правила ранжирования.',
	...
	tool_onpage_rules_item_2:
		'H1: странице нужен заголовок, который называет её тему. Идеального числа заголовков у Google нет, и за лишние H1 никто не наказывает, поэтому несколько H1 помечаются только ради ясной структуры и доступности.',
	tool_onpage_rules_item_3:
		'Canonical: тег <link rel="canonical"> в <head> — подсказка о предпочтительном URL среди дублей, а не директива. Google взвешивает её вместе с редиректами, картами сайта и внутренними ссылками, а выбор делает сам.',
	...
	tool_onpage_faq_q2: 'Вредит ли SEO несколько тегов H1?',
	tool_onpage_faq_a2:
		'Нет. Google прямо говорит, что идеального числа заголовков не существует, и ранжирует страницы вообще без H1, с одним и с несколькими. Один H1 стоит оставлять ради доступности и понятной структуры — плюс так больше шансов, что Google возьмёт ваш заголовок в заголовок сниппета.',
	tool_onpage_faq_q3: 'Должен ли canonical указывать на сам себя?',
	tool_onpage_faq_a3:
		'Не обязательно, но для страницы, которая должна ранжироваться сама, самоуказывающий canonical — самый понятный сигнал. Указание на другой URL объявляет эту страницу дублем. В любом случае Google считает canonical подсказкой и может выбрать другой URL.',
