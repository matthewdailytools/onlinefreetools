...
const ru: SiteLangDict = {
	...
	tool_og_description:
		'Нет OG-картинки или title — меньше кликов при шаринге. Вставьте og/twitter или URL для карточек Facebook, X и чатов. Вставка локально. Пример: 1200×630.',
	...
	tool_og_rules_item_1: 'Протокол Open Graph требует четыре поля: og:title, og:type, og:image и og:url. og:description в спецификации необязателен, но его выводят все крупные платформы — если его не задать, платформа сама составит описание из текста страницы.',
	...
	tool_og_faq_a1:
		'Facebook, X и WhatsApp подставляют HTML-заголовок, meta description и любую найденную на странице картинку. Такая подстановка у каждой платформы своя и со временем меняется, поэтому карточка может выйти неполной или с неверной картинкой — для этого поля og: и прописывают вручную.',
