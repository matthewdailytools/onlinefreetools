...
const ja: SiteLangDict = {
	...
	tool_robots_rules_item_1: 'グループの選び方：クローラーは User-agent が最も具体的に一致するグループに従います。自分の名前が書かれていれば * より優先され、ファイル内の記述順は判断材料になりません。同じクローラー名の User-agent 行が複数あれば 1 つのグループに統合されます（RFC 9309）。',
	tool_robots_rules_item_2: 'プレフィックス一致：Allow と Disallow はパスプレフィックスで照合され、部分一致や正規表現ではありません。特殊文字は * と $ のみ（RFC 9309）。',
	tool_robots_rules_item_3: 'グループ内の優先順位：一致したパスのうち最も長いものが勝ちます。Allow と Disallow の一致長が同じときは Allow が勝ちます。つまり Disallow: /admin/ と Allow: /admin/public/ を併記すれば、public 配下だけクロールを残せます。',
	tool_robots_rules_item_4: 'サイト全体と Sitemap 行：Disallow: / はそのエージェントをすべての URL からブロックし、値が空の Disallow はすべて許可します。Sitemap: は RFC 9309 の範囲外の拡張で、大文字小文字を問わずどこに書いても構いません。このツールは末尾に置きます。',
	...
	tool_robots_usecase_2: 'AI クローラーの制御：Googlebot のクロールは残したまま、GPTBot・ClaudeBot・Google-Extended・CCBot・PerplexityBot に専用の Disallow グループを与えます。なお Google-Extended の対象は Gemini と Vertex AI のグラウンディングで、検索の「AI による概要」は引き続き Googlebot 経由です。',
	tool_robots_usecase_3: '発見性向上：robots.txt に Sitemap 行を入れて、クローラーがサイトマップを見つけやすくする。',
	tool_robots_faq_q1: 'Disallow: / と書くとどうなりますか？',
	tool_robots_faq_a1: 'そのクローラーにサイトルート配下の URL を取得しないよう伝えます。Googlebot に Disallow: / を設定すると、ページが Google 検索から消える可能性があります。ただし Disallow が止めるのはクロールで、インデックス登録ではありません。ブロックした URL でも外部からリンクされていればスニペットなしで結果に残ることがあるので、確実に消したいときはページをクロールできる状態にして noindex を付けます。ステージングなら Disallow: /private/ のように具体的なパスで指定してください。',
