...
const zh: SiteLangDict = {
	tool_sketch_prompt_generator_article:
		'在本页构建可粘贴的 素描 Prompt 生成器 Prompt。填写字段，复制 Markdown 或 JSON 到 ChatGPT、Gemini、Claude 或 DeepSeek。默认本地组装；可选 Expand/Polish 使用 Cloudflare Workers AI（Turnstile）。不用 AI 时文本留在本机。',
	...
	tool_sketch_prompt_generator_desc:
		'素描 Prompt 生成器 — 默认本地组装，可选 Cloudflare AI 扩写/润色（Turnstile）；Markdown/JSON 留在本机。',
	...
	tool_sketch_prompt_generator_faq_a3:
		'Prompt 模板构建器 covers generic Role/Task/Constraints/Output templates. This page focuses on sketch prompt generator fields with a dedicated sample and rules for this scenario.',
	...
	tool_sketch_prompt_generator_how_body:
		'填写 素描 Prompt 生成器 字段，本地构建 Prompt，可选 Turnstile 后 Expand/Polish，再粘贴到 ChatGPT、Gemini、Claude 或 DeepSeek。',
	...
	tool_sketch_prompt_generator_rules_body:
		'素描 Prompt 生成器：默认本地组装；可选 Cloudflare AI 有频率限制且须 Turnstile。',
	...
	tool_sketch_prompt_generator_title:
		'素描 Prompt 生成器 — 本地 + 可选 AI',
	tool_sketch_prompt_generator_usecase_1:
		'为团队在 ChatGPT、Gemini、Claude 或 DeepSeek 准备可粘贴的 素描 Prompt 生成器 简报。',
