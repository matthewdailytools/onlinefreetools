...
const ja: SiteLangDict = {
  ...
  tool_transcribe_an_audio_file_to_text_description: 'ブラウザの SpeechRecognition がある場合、ローカル音声を文字起こしできます。手順：音声を選ぶ、言語を選ぶ、再生しながら起こすまたはマイク口述、編集、TXT を保存。例：サンプル読込は短いトーンで再生パスを実行—実音声ファイルかマイクが必要。Whisper クローンではなく巨大 wasm も読み込みません。多くのブラウザはアップロードファイルの確実なオフライン ASR を持ちません。このページはサーバー ASR のためにファイルをアップロードしません。',
  ...
  tool_transcribe_an_audio_file_to_text_faq_q1: 'Whisper クローンやクラウド ASR アップロードですか？',
  tool_transcribe_an_audio_file_to_text_faq_a1: 'いいえ。Whisper wasm は同梱せず、ASR のために当サーバーへアップロードもしません。利用可能な場合はブラウザの SpeechRecognition を使います。',
