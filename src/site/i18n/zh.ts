...
const zh: SiteLangDict = {
  ...
  tool_prompt_template_builder_ai_consent_body: '此可选步骤会把当前草稿发往 Cloudflare Workers AI 推理；不会从我们的服务器调用 OpenAI、Google、Anthropic 或 DeepSeek。不用 AI 也能本地组装。',
  ...
  tool_prompt_template_builder_faq_a2: '本地模式只在本标签页整理文字，不调用 ChatGPT、Gemini、Claude、DeepSeek 或任何模型 API。可选 AI 使用 Cloudflare Workers AI（见下方 FAQ）。',
  tool_prompt_template_builder_faq_a3: 'ChatGPT 导出转 Markdown 是把对话导出变成可读文件。本页是把草稿整理成可复用的系统 Prompt——归档对话后可再用本页提炼模板。',
  tool_prompt_template_builder_faq_a4: '可以。JSON 芯片输出 {role,task,constraints,output}，便于配置或测试。需要 Schema 校验时可配合 JSON Schema 校验器。',
  ...
  tool_writing_prompt_generator_faq_a1: '不会。Prompt 只在本浏览器标签页组装。不会上传到 OpenAI、Google、Anthropic、DeepSeek 或我们的服务器。',
  tool_writing_prompt_generator_faq_a2: '不会。本页只把你的写作 brief 格式化成 Prompt 块，不调用 ChatGPT、Gemini、Claude、DeepSeek 或任何模型 API。',
  ...
  tool_writing_prompt_generator_faq_a4: '可以。随机模式摇出体裁、场景、物件、情绪与冲突。可选数字 seed 可复现同一组结果。随机输出仅供灵感，不保证质量。',
  ...
  tool_writing_prompt_generator_faq_a6: '可以。把成品 Prompt 复制到 ChatGPT、Gemini、Claude 或 DeepSeek。我们不按平台拆独立 URL，因为任务是整理文本，不是代调用 API。',
