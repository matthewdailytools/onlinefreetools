...
- 工具做什么：在单张本地图上叠加文字水印（位置/透明度/旋转）并下载。
- 谁在什么任务里用：摄影作品署名、截图标注「样张」、防盗用提示。
...
**选定主方向**：A  
**次要互链参考**：`image-overlay`, `image-exif`, `image-border`
...
- 成熟度 / 推荐包：Canvas fillText；字号/颜色/alpha/旋转/九宫格锚点
- Tier：**0**
...
- 继续立项：是
- 建议 slug：`add-watermark`（`/tools/add-watermark`）
- 与已上线工具关系：**新建**；catalog `image`；Related：`image-overlay`, `image-exif`, `image-border`
