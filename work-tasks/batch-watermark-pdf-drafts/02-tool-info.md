# Batch watermark PDF drafts

Status: implemented; direction A; YMYL: no; slug: `batch-watermark-pdf-drafts`; page.style: opts.
...
Auto sample includes different page sizes and rotation. A watermark is a visual label, not protection or redaction.
...
How: choose input, review settings, process, inspect output/report, download. Why: a specific output, explicit limits/failure handling, and local verification of results. Example uses real generated input through the production pipeline. FAQ covers privacy, output limitations and unsupported files. Related: `pdf-watermark` plus existing same-topic tools; references: https://pdf-lib.js.org/docs/api/classes/pdfpage.

## 清单前检索覆盖
| 项目 | 结论 |
| --- | --- |
| 主检索词 → title/H1 | Batch watermark PDF drafts |
| 优化摘要 | 将泛格式转换主词缩为真实输入和输出任务；批处理、检查结果及失败行为写入说明，不把设置堆进标题。 |
| slug 结论 | 保留 `batch-watermark-pdf-drafts`，与邻页 `pdf-watermark` 的主要产物或批量验收边界区分。 |
- [x] 已回写开发 / SEO 卡片：采用上面的标题、输出边界及按钮顺序。

## 用户意图审查
| 项目 | 判断 |
| --- | --- |
| 总判 | 满足用户明确提出的第 3 项任务；范围及失败规则已进入交互规格，未承诺无损还原或完美识别。 |

## 同意图相关搜索词
| 相关搜法 | 判定 | 页面生成落点 |
| --- | --- | --- |
| Batch watermark PDF drafts | 主词 | H1 |
| watermark multiple PDFs | absorb | desc |
| 批量 PDF 水印 | absorb | title / How |
| 多份 PDF 加水印 | absorb | FAQ / usecase |
- [x] 上表已列全本次审查范围的同意图词；没有将此表冒充完整 SERP 数据。
- [x] 生成文案时按上表写入对应落点。

## Evidence and distinction
Current site neighbor: `pdf-watermark`. Technical primary reference: https://pdf-lib.js.org/docs/api/classes/pdfpage. The planning session inspected public batch-watermark and batch-SVG product pages, not a complete ranked SERP. No unverified traffic or competitive superiority claims.
