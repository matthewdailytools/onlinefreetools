...
**路径**：`/tools/check-pdf-a-compliance`  
**主方向**：A  
**YMYL**：否  
...
- 目标主词 / 长尾意图：check pdf/a compliance; related Planner absorb terms
- 用户真实任务：Parse PDF; run client-side rule checks (version, fonts, metadata); pdf check / verapdf intent.
...
| 字段 | 内容 |
|---|---|
| 集群 / 优先级 | pdf scene N17 |
| 技术 | pdf-lib / pdf.js CDN；`page.style: opts` |
| Title (en) / H1 | **Check PDF/A compliance** |
| Description 要点 | Parse PDF; run client-side rule checks (version, fonts, metadata); pdf check / verapdf intent. Files stay on your device; not uploaded to a server. Example in page. |
| FAQ | ≥3 场景/边界/邻页划界 |
...
|---|---|
| 日期 | 2026-09-03 |
| slug 结论 | 保留 `check-pdf-a-compliance` |
| 主检索词 → title/H1 | **Check PDF/A compliance** |
