...
- 目标主词 / 长尾意图：convert pdf to pdf/a; related Planner absorb terms
- 用户真实任务：Rewrite with PDF/A metadata + output profile; limits in Rules.
...
| 字段 | 内容 |
|---|---|
| 集群 / 优先级 | pdf scene N16 |
| 技术 | pdf-lib / pdf.js CDN；`page.style: opts` |
| Title (en) / H1 | **Convert PDF to PDF/A** |
| Description 要点 | Rewrite with PDF/A metadata + output profile; limits in Rules. Files stay on your device; not uploaded to a server. Example in page. |
| FAQ | ≥3 场景/边界/邻页划界 |
...
|---|---|
| 日期 | 2026-09-03 |
| slug 结论 | 保留 `convert-pdf-to-pdf-a` |
| 主检索词 → title/H1 | **Convert PDF to PDF/A** |
...
|---|---|---|---|---|
| convert pdf to pdf/a | build 主词 | H1 | 进页默认该场景 | |
...
|---|---|
| 日期 | 2026-09-03 |
| 总判 | 满足：进页即 convert pdf to pdf/a 场景 |
| 主词搜索者任务 | Rewrite with PDF/A metadata + output profile; limits in Rules. |
...
- 输入/输出：Rewrite with PDF/A metadata + output profile; limits in Rules.
- 核心规则 / 算法：见 Page 实现
