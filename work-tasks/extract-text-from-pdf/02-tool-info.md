...
**路径**：`/tools/extract-text-from-pdf`  
**主方向**：A  
**YMYL**：否  
...
- 目标主词 / 长尾意图：extract text from a pdf; related Planner absorb terms
- 用户真实任务：pdf.js text extract; copy/download .txt.
...
| 字段 | 内容 |
|---|---|
| 集群 / 优先级 | pdf scene N19 |
| 技术 | pdf-lib / pdf.js CDN；`page.style: opts` |
| Title (en) / H1 | **Extract text from a PDF** |
| Description 要点 | pdf.js text extract; copy/download .txt. Files stay on your device; not uploaded to a server. Example in page. |
| FAQ | ≥3 场景/边界/邻页划界 |
...
|---|---|
| 日期 | 2026-09-03 |
| slug 结论 | 保留 `extract-text-from-pdf` |
| 主检索词 → title/H1 | **Extract text from a PDF** |
...
|---|---|---|---|---|
| extract text from a pdf | build 主词 | H1 | 进页默认该场景 | |
...
|---|---|
| 日期 | 2026-09-03 |
| 总判 | 满足：进页即 extract text from a pdf 场景 |
| 主词搜索者任务 | pdf.js text extract; copy/download .txt. |
...
- 输入/输出：pdf.js text extract; copy/download .txt.
- 核心规则 / 算法：见 Page 实现
