...
|---|---|
| 日期 | 2026-09-02 |
| slug | 保留 `film-prompt-builder` |
| 主检索词 → H1 | **Film prompt builder** |
| 次词 | screenplay beat sheet → how_item_2; three-act → rules_item_1 |
| 用户习惯 | 搜 film prompt builder 要进页即用字段 + 样例 + 本地组装 |
| 优化摘要 | H1 锁定 Film prompt builder — Local + Optional AI；次词落 desc/FAQ/use cases；vs prompt-template-builder FAQ 划界 |
...
| 主任务 | film prompt builder 本地组装并粘贴到 ChatGPT 等 |
...
| 字段 | 内容 |
|---|---|
| Title (en) / H1 | **Film prompt builder — Local + Optional AI** |
...
- 字段：logline, act1, act2, act3, scene_list, character_arc
