...
- 工具做什么：水平或垂直翻转一张本地图片并下载。
- 谁在什么任务里用：修正自拍镜像、排版左右对调、竖图上下翻转。
...
**选定主方向**：A  
**次要互链参考**：`image-crop`, `image-grayscale`, `image-border`
...
- 成熟度 / 推荐包：Canvas scale(-1,1)/(1,-1) 再 drawImage；导出 PNG/JPEG/WebP
- Tier：**0**
...
- 继续立项：是
- 建议 slug：`flip-image`（`/tools/flip-image`）
- 与已上线工具关系：**新建**；catalog `image`；Related：`image-crop`, `image-grayscale`, `image-border`
