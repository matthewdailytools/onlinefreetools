...
- 工具做什么：在浏览器内把图片与 Base64（data URL / 纯串）互相转换，不上传。
- 谁在什么任务里用：前端/邮件模板作者嵌入图、调试 data URL、粘贴 Base64 还原预览。
...
- 成熟度 / 推荐包：FileReader.readAsDataURL / fetch(dataURL)+blob；可选 Canvas 重编码 MIME
- Tier：**0**
...
- 继续立项：是
- 建议 slug：`image-to-base64`（`/tools/image-to-base64`）
- 与已上线工具关系：**新建**；catalog `image`；Related：`image-format-converter`, `image-compress`
