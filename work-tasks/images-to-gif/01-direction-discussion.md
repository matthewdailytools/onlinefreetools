...
- 工具做什么：多张本地图片按序合成动画 GIF 并下载。
- 谁在什么任务里用：简单动图、步骤演示、表情包草图。
...
**选定主方向**：A  
**次要互链参考**：`images-to-pdf`, `image-merge`, `image-format-converter`
...
- 成熟度 / 推荐包：懒加载 gifenc（或等价）；帧延迟/循环；统一画布尺寸 contain
- Tier：**1**
...
- 继续立项：是
- 建议 slug：`images-to-gif`（`/tools/images-to-gif`）
- 与已上线工具关系：**新建**；catalog `image`；Related：`images-to-pdf`, `image-merge`, `image-format-converter`
