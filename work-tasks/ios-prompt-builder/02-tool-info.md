...
|---|---|
| 日期 | 2026-09-02 |
| slug | 保留 `ios-prompt-builder` |
| 主检索词 → H1 | **iOS prompt builder** |
| 次词 | SwiftUI coding agent → desc; Apple HIG → rules_item_3 |
| 用户习惯 | 搜 ios prompt builder 要进页即用字段 + 样例 + 本地组装 |
| 优化摘要 | H1 锁定 iOS prompt builder — Local + Optional AI；次词落 desc/FAQ/use cases；vs prompt-template-builder FAQ 划界 |
...
| 主任务 | ios prompt builder 本地组装并粘贴到 ChatGPT 等 |
...
| 字段 | 内容 |
|---|---|
| Title (en) / H1 | **iOS prompt builder — Local + Optional AI** |
...
- 字段：feature_spec, swift_stack, swiftui_ui, hig_constraints
