...
- [x] 缺口：键序 vs 语义；数组列表 vs 集合；非法 JSON。
...
| 满足之处 | loadSample 键序不同但值相同 → 无差；第二组改值 → 有路径 |
...
- 输入：双栏 JSON textarea；开关「忽略键序」（默认开）；「数组当集合」默认关
- 输出：路径列表 added/removed/changed
- 核心：JSON.parse；规范化后递归
