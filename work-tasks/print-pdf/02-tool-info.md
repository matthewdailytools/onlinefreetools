...
- 目标主词 / 长尾意图：print a pdf; related Planner absorb terms
- 用户真实任务：Load PDF; embedded viewer + window.print().
...
| 字段 | 内容 |
|---|---|
| 集群 / 优先级 | pdf scene N21 |
| 技术 | pdf-lib / pdf.js CDN；`page.style: opts` |
| Title (en) / H1 | **Print a PDF** |
| Description 要点 | Load PDF; embedded viewer + window.print(). Files stay on your device; not uploaded to a server. Example in page. |
| FAQ | ≥3 场景/边界/邻页划界 |
...
|---|---|
| 日期 | 2026-09-03 |
| slug 结论 | 保留 `print-pdf` |
| 主检索词 → title/H1 | **Print a PDF** |
...
|---|---|---|---|---|
| print a pdf | build 主词 | H1 | 进页默认该场景 | |
...
|---|---|
| 日期 | 2026-09-03 |
| 总判 | 满足：进页即 print a pdf 场景 |
| 主词搜索者任务 | Load PDF; embedded viewer + window.print(). |
...
- 输入/输出：Load PDF; embedded viewer + window.print().
- 核心规则 / 算法：见 Page 实现
