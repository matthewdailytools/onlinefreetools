...
**路径**：`/tools/short-drama-prompt-generator`  
**主方向**：B  
**YMYL**：否  
...
| 场景 | 进页默认 | 说明 |
|---|---|---|
| Short drama prompt generator | **是** | H1 |
...
|---|---|
| 日期 | 2026-09-02 |
| slug | 保留 `short-drama-prompt-generator` |
| 主检索词 → H1 | **Short drama prompt generator** |
| 次词 | vertical serial outline → desc; cliffhanger hooks → FAQ q4 |
| 用户习惯 | 搜 short drama prompt generator 要进页即用字段 + 样例 + 本地组装 |
| 优化摘要 | H1 锁定 Short drama prompt generator — Local + Optional AI；次词落 desc/FAQ/use cases；vs prompt-template-builder FAQ 划界 |
...
| 主任务 | short drama prompt generator 本地组装并粘贴到 ChatGPT 等 |
...
| 字段 | 内容 |
|---|---|
| Title (en) / H1 | **Short drama prompt generator — Local + Optional AI** |
...
- 字段：episodes, hook, cliffhanger, vertical_format, genre
