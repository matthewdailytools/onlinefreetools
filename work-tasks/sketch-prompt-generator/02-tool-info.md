...
|---|---|
| 日期 | 2026-09-02 |
| slug | 保留 `sketch-prompt-generator` |
| 主检索词 → H1 | **Sketch prompt generator** |
| 次词 | pencil sketch prompt → FAQ q4; art study → use case 2 |
| 用户习惯 | 搜 sketch prompt generator 要进页即用字段 + 样例 + 本地组装 |
| 优化摘要 | H1 锁定 Sketch prompt generator — Local + Optional AI；次词落 desc/FAQ/use cases；vs prompt-template-builder FAQ 划界 |
...
| 主任务 | sketch prompt generator 本地组装并粘贴到 ChatGPT 等 |
...
| 字段 | 内容 |
|---|---|
| Title (en) / H1 | **Sketch prompt generator — Local + Optional AI** |
...
- 字段：medium, stroke, composition, artist_ref
