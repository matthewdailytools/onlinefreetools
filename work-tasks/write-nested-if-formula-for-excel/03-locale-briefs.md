...
**工具 slug**：`write-nested-if-formula-for-excel`  
**母版语言**：en  
**状态**：`i18n-done`
...
- 检索词（3–5）：write a nested IF formula for Excel；本工具场景句；隐私「不上服务器」
- 语气：口语工具站
- Title / H1 方向：当地检索向场景句（en 母版 **Write a nested IF formula for Excel**），非参数枚举
...
| 轮次 | 日期 | 做了什么 | 结果 |
|---|---|---|---|
| 0b 清单前检索覆盖 | 2026-09-04 | slug 用场景 kebab；H1=Write a nested IF formula for Excel；次词进 FAQ；Planner 长尾写入 02 表 | 覆盖表已填；intent 总判满足；主词在 H1 |
...
| IG 修正复审 | 2026-09-05 | en/zh/es/ja 重写任务步骤与完整公式；其余语补真实 Input→Output；补本地 Excel 语法边界 | 等于试算改为不区分大小写；无按钮口号 |
